...
void stageFive(){
	...
	// int wordClassification[NUM_WORD_TYPES];
	// while (word != NULL){
	// 	betterClassify(word->value, wordClassification,
	// 		&dictionary, NUM_WORD_TYPES);
	}
...
int addToDictionary(DictionaryType *dict, DictEntryType newEntry,
	...
	/* I forgot to assume that the input is already sorted, hence this part
	actually sorts the input, but oh well */
...
void classify(char *word, int *output, DictionaryType *dictionary,
	...
	int defaultProbabilities[NUM_WORD_TYPES] = {0,0,1};

	if (pos >= 0) {
		int *probs = dictionary->entries[pos].probabilities;
		int i;
		for (i = 0; i < NUM_WORD_TYPES; i++) {
			defaultProbabilities[i] = probs[i] > 0;
		}
		defaultProbabilities[NUM_WORD_TYPES - 1] = 0;
	} 
	copyArray(defaultProbabilities, output, NUM_WORD_TYPES);
}

int search(char *str, DictEntryType entries[], int itemCount) {
	/* TODO: CHANGE THIS TO BINARY SEARCH LATER */
	int i = itemCount/2;
	int compare = strcmp(str,entries[i].word);
	...
	if (compare == 0) {
		...
		search(str, entries, itemCount/2);
	} else if (compare > 0) {
		search(str, entries + i + 1, itemCount/2);
	}
